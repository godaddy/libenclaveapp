...
- **Use the key without biometrics** (when access policy is set, and the consent gate is hardware-enforced). On macOS the Secure Enclave enforces Touch ID inside `SecKeyCreateSignature` and software cannot bypass this. On Windows the picture is split: the **WebAuthn / SK path** (`enclaveapp-windows-webauthn`) is hardware-enforced — the TPM via NGC will not produce a signature without an OS-mediated Hello gesture. The **legacy Platform-KSP path** uses user-mode `UserConsentVerifier` for the prompt (see "FFI trust boundaries" below), so the `Verified` Boolean is in principle hookable by an attacker with code execution as the user — biometrics are a strong UX deterrent on that path but not a hard cryptographic gate. Apps that need hardware-enforced consent on Windows must use the SK path (sshenc does so by default when Hello is enrolled, since v0.6.44).
...
- **Connect to the agent socket and request signatures** (Type 1). Socket permissions and peer UID verification prevent cross-user access, but same-user access is inherent to Unix domain sockets / Windows named pipes. The defense against unlimited signatures is the access-policy / user-presence gate — Touch ID on macOS and the WebAuthn-Hello path on Hello-enrolled Windows are hardware-enforced; the Windows legacy Platform-KSP path uses a user-mode `UserConsentVerifier` Boolean that is in principle hookable. Without any access policy at all, an attacker who can reach the agent gets unlimited signatures regardless of platform.
...
Hardware access policies (Touch ID on macOS, Windows Hello via the WebAuthn / SK path) are the only defense against root — they require physical presence that root cannot fake. The Windows legacy Platform-KSP path's `UserConsentVerifier` prompt is shown to the user but the resulting `Verified` Boolean is in user-mode and a kernel-capable root can hook it; root is therefore a stronger threat against the legacy path than against the SK path or macOS Secure Enclave.
...
- **Windows Platform-KSP path: NCRYPT UI policy is best-effort, not hardware-gated.** `ui_policy::verify_ui_policy_matches` reads the CNG key's actual `NCRYPT_UI_POLICY` via `NCryptGetProperty` to detect attacker-planted keys with mismatched UI policy. Historically (pre-PR #99) this was the load-bearing hardware gate, but on Hello-enrolled Win11 hosts `NCRYPT_UI_PROTECT_KEY_FLAG` surfaces as the legacy CryptUI password dialog rather than Hello biometric, which is both a UX regression and a misleading security signal. PR #99 dropped the flag at key-creation time when Hello is detected, so the legacy Platform-KSP path no longer has a TPM-enforced consent gate — it relies on user-mode `UserConsentVerifier` (WinRT) for the prompt, and the `Verified` Boolean is hookable by an attacker with code execution as the user. Apps that need hardware-enforced consent on Windows (e.g., sshenc since v0.6.43) use `enclaveapp-windows-webauthn` instead, which routes through `WebAuthNAuthenticatorGetAssertion` / TPM-via-NGC; the OS will not produce a signature without an OS-mediated Hello gesture, and the gate is enforced outside user-mode. See `enclaveapp-windows-webauthn/src/lib.rs` for the threat model of that crate. The CNG `verify_ui_policy_matches` call remains as a tamper detector for the legacy path: a planted key with an *unexpected* UI flag still gets refused, just don't conflate "matches metadata" with "hardware-enforced".
