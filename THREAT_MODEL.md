...
**Linux TPM backend:** `AccessPolicy` is stored in key metadata and displayed to the user, but `LinuxTpmSigner::sign` does not consult it. The TPM key is created with empty authorization; every sign operation succeeds without a user prompt regardless of the policy. A key created with `AccessPolicy::BiometricOnly` on Linux TPM provides no biometric gate — it behaves identically to `AccessPolicy::None` at runtime. Applications that require enforced user presence on Linux should not rely on this backend to supply it.
