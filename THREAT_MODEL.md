...
- **`.meta` HMAC sidecar on the keyring backend.** `metadata::save_meta_with_hmac` / `load_meta_with_hmac` write and verify a `<label>.meta.hmac` sidecar keyed by a per-app random HMAC key held in the system keyring (`enclaveapp_keyring::meta_hmac_key`). `enclaveapp-app-storage::ensure_key` verifies the sidecar on Linux and rejects HMAC-mismatched loads with `meta_hmac_verify` — a same-UID attacker who edits `<label>.meta` to flip `BiometricOnly` → `None` without also having keyring access is caught. Apple SE and Windows CNG do not write the sidecar in that older path because those chips enforce the access policy at key-creation time; `.meta` tamper changes what the UI displays but cannot bypass the hardware gate. Linux TPM does not write the sidecar either, but for a different reason: the Linux TPM backend does not enforce `AccessPolicy` at sign time (see "Linux TPM backend" below). The sidecar is absent for pre-upgrade keys — `load_meta_with_hmac` falls through to `load_meta` verbatim in that case, so existing installs migrate transparently on the next key regeneration.
- **`.meta` per-key trust anchor on macOS (Apple SE).** Layered on top of the sidecar, the Apple backend writes a 32-byte HMAC-SHA256 tag of `<label>.meta` to a per-key Keychain item (`com.godaddy.<app>.meta-tag` / `<label>`) at keygen time via `enclaveapp-apple::meta_tag::store`. The agent's per-op load (`load_handle_with_context`, `load_pub_key`) verifies the on-disk meta against the keychain-stored tag through `ensure_meta_integrity`. Tamper (mismatch) and `legacy_meta` (no tag) both refuse the operation. The on-disk sidecar becomes a derivable cache; the keychain tag is the authority. This closes the auto-migrate hole the prior sidecar-only design left open: a same-UID attacker who `rm`'d the sidecar to force a re-bless of tampered `.meta` is now caught, because the keychain tag the attacker can't read or write is what the verifier compares against. Migration to the trust anchor for pre-existing keys is gated by an explicit user-confirmed `<app> migrate-meta` subcommand, with a Keychain-backed completion marker (`com.godaddy.<app>.migrate-marker` / `__completed__`) that the agent consults to switch the `legacy_meta` error message between gentle (one-time-cutover) and strong (tamper-signal) variants. The verify path uses `meta_hmac::load_existing` (read-only) so it can never trigger a `SecItemAdd` and hang on a locked keychain. Extending the trust anchor to Windows TPM and Linux keyring/TPM is a candidate follow-up; until that lands, those platforms still rely on the older sidecar-only path. See `crates/enclaveapp-apple/docs/design-meta-hmac-trust-anchor.md` (file lives under libenclaveapp/docs/) for the full design.
