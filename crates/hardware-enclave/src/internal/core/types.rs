...
pub fn validate_label(label: &str) -> crate::internal::core::Result<()> {
    if label.is_empty() {
        return Err(crate::internal::core::Error::InvalidLabel {
...
pub fn validate_p256_point(bytes: &[u8]) -> crate::internal::core::Result<()> {
    if bytes.len() != 65 {
