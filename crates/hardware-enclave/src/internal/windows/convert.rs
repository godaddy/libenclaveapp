...
pub fn der_to_p1363(der: &[u8]) -> crate::internal::core::Result<Vec<u8>> {
    if der.len() < 6 {
...
/// Output: `0x04 || X || Y`.
pub fn eccpublic_blob_to_sec1(blob: &[u8]) -> crate::internal::core::Result<Vec<u8>> {
    if blob.len() < 8 {
        return Err(Error::KeyOperation {
...
pub fn sec1_to_eccpublic_blob(point: &[u8], magic: u32) -> crate::internal::core::Result<Vec<u8>> {
...
fn parse_der_integer(data: &[u8]) -> crate::internal::core::Result<(Vec<u8>, usize)> {
    if data.len() < 2 {
        return Err(Error::KeyOperation {
